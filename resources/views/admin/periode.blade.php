    ...
    <div id="modal-edit" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <form action="{{ route('admin.periode.edit') }}" method="post">
                    @csrf
                    @method('put')
                    <input type="hidden" name="periode_id" id="periode_id_edit">
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <div class="col-span-2">
                            <label for="tahun_edit" class="block mb-2.5 text-sm font-medium text-heading">Tahun</label>
                    ...
                    <div class="flex items-center space-x-4 border-t border-default pt-4 md:pt-6">
                        <button type="submit"
                            class="inline-flex items-center text-white bg-blue-500 box-border border border-transparent focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none hover:bg-blue-700 transition duration-200 ease-in-out">
                            Simpan Perubahan
                        </button>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 id="text-modal-hapus" class="mb-6 text-body">Apakah anda yakin akan menghapus data periode ini?</h3>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 id="text-modal-aktivasi" class="mb-6 text-body">Apakah anda yakin ingin mengubah status periode ini?</h3>
                    <form action="{{ route('admin.periode.aktivasi') }}" method="post">
                        @csrf
                        @method('put')
                        <input type="hidden" name="periode_id" id="periode_id_aktivasi">
                        <div class="flex items-center space-x-4 justify-center">
                            <button type="submit"
                                class="text-white transition duration-300 ease-in-out bg-blue-500 box-border border border-transparent hover:bg-blue-700 focus:ring-4 focus:ring-danger-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">
                                Iya, lanjutkan
                            ...
                            <button data-modal-hide="modal-aktivasi" type="button"
                                class="text-body transition duration-300 ease-in-out bg-white box-border border border-default-medium hover:bg-gray-200 hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">Batal</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    @push('js')
        <script>
            $(document).on('click', '.button-edit', function() {
                const tahun = String($(this).data('tahun'));

                $('#periode_id_edit').val($(this).data('id'));
                if ($('#tahun_edit option[value="' + tahun + '"]').length === 0) {
                    $('#tahun_edit').append(new Option(tahun, tahun));
                }
            ...
            $(document).on('click', '.button-hapus', function() {
                ...
                $('#text-modal-hapus').text('Apakah anda yakin akan menghapus periode ' + $(this).data('tahun') + '?');
            ...
            $(document).on('click', '.button-aktivasi', function() {
                ...
                $('#periode_id_aktivasi').val($(this).data('id'));
                $('#text-modal-aktivasi').text('Apakah anda yakin ingin ' + aksi + ' periode ' + tahun + '?');
