    ...
    <div id="modal-tambah" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.prodi.tambah') }}" method="post">
                    @csrf
                    @method('post')
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <div class="col-span-2">
                            <label for="nama_prodi" class="block mb-2.5 text-sm font-medium text-heading">Nama Prodi</label>
                            <input type="text" name="nama_prodi" id="nama_prodi"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="kode_prodi" class="block mb-2.5 text-sm font-medium text-heading">Kode Prodi</label>
                            <input type="text" name="kode_prodi" id="kode_prodi"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="jenjang" class="block mb-2.5 text-sm font-medium text-heading">Jenjang</label>
    ...
    <div id="modal-edit" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.prodi.edit') }}" method="post">
                    @csrf
                    @method('put')
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <input type="hidden" name="prodi_id" id="prodi_id_edit"
                            class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                            required="">
                        <div class="col-span-2">
                            <label for="nama_prodi" class="block mb-2.5 text-sm font-medium text-heading">Nama Prodi</label>
                            <input type="text" name="nama_prodi" id="nama_prodi_edit"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="kode_prodi" class="block mb-2.5 text-sm font-medium text-heading">Kode Prodi</label>
                            <input type="text" name="kode_prodi" id="kode_prodi_edit"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="jenjang"
                                class="block mb-2.5 text-sm font-medium text-heading">Jenjang</label>
                            <input type="text" name="jenjang" id="jenjang_edit"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                    </div>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-6 text-body">Apakah anda yakin akan menghapus data prodi ini?</h3>
                    <form action="{{ route('admin.prodi.hapus') }}" method="post">
                        @csrf
                        @method('delete')
                        <input type="text" name="prodi_id" id="prodi_id_hapus" hidden>
...
<script>
    $(document).on('click', '.button-edit', function() {
        let prodi_id = $(this).attr('data-id');
        let nama_prodi = $(this).attr('data-nama-prodi');
        let kode_prodi = $(this).attr('data-kode-prodi');
        let jenjang = $(this).attr('data-jenjang');

        $('#prodi_id_edit').val(prodi_id);
        $('#nama_prodi_edit').val(nama_prodi);
        $('#kode_prodi_edit').val(kode_prodi);
