<x-app-layout>
            ...
            <div class="relative overflow-x-auto bg-white shadow-xs rounded-lg border border-default">
                <div class="flex justify-between items-center py-4 mx-4 border-b border-gray-300">
                    <button data-modal-target="modal-tambah" data-modal-toggle="modal-tambah"
                        ...
                        type="button">
                        <i class="bi bi-plus"></i> <span class="text-sm">Tambah Sub Standar Mutu</span>
                    </button>
                </div>
    ...
    <div id="modal-tambah" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.sub_standar_mutu.tambah') }}" method="post">
                    @csrf
                    @method('post')
                    <input type="hidden" name="standar_mutu_id" value="{{ $standar_mutu_id }}">
    ...
    <div id="modal-edit" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.sub_standar_mutu.edit') }}" method="post">
                    @csrf
                    @method('put')
                    <input type="hidden" name="standar_mutu_id" value="{{ $standar_mutu_id }}">
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <form action="{{ route('admin.sub_standar_mutu.hapus') }}" method="post">
                        @csrf
                        @method('delete')
                        <input type="hidden" name="standar_mutu_id" value="{{ $standar_mutu_id }}">
                        <input type="text" name="sub_standar_id" id="sub_standar_id_hapus" hidden>
    ...
    <script>
        $(document).on('click', '.button-edit', function() {
            let sub_standar_id = $(this).data('id');
            let nama_sub_standar_mutu = $(this).data('nama');

            $('#sub_standar_id').val(sub_standar_id);
            $('#nama_sub_standar_mutu').val(nama_sub_standar_mutu);
        ...
        $(document).on('click', '.button-hapus', function() {
            let sub_standar_id = $(this).data('id');
            $('#sub_standar_id_hapus').val(sub_standar_id);
