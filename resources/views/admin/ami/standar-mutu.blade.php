<x-app-layout>
            ...
            <div class="relative overflow-x-auto bg-white shadow-xs rounded-lg border border-default">
                <div class="flex justify-between items-center py-4 mx-4 border-b border-gray-300">
                    <button data-modal-target="modal-tambah" data-modal-toggle="modal-tambah"
                        ...
                        type="button">
                        <i class="bi bi-plus"></i> <span class="text-sm">Tambah Standar Mutu</span>
                    </button>
                </div>
    ...
    <div id="modal-tambah" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.standar_mutu.tambah') }}" method="post">
                    @csrf
                    @method('post')
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <div class="col-span-2">
                            <label for="name" class="block mb-2.5 text-sm font-medium text-heading">Nama Standar Mutu</label>
                            <input type="text" name="nama_standar_mutu" id="name"
    ...
    <div id="modal-edit" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form action="{{ route('admin.standar_mutu.edit') }}" method="post">
                    @csrf
                    @method('put')
                    <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                        <input type="hidden" name="standar_mutu_id" id="standar_mutu_id"
                            class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                            required="">
                        <div class="col-span-2">
                            <label for="name" class="block mb-2.5 text-sm font-medium text-heading">Nama Standar Mutu</label>
                            <input type="text" name="nama_standar_mutu" id="nama_standar_mutu"
                                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                required="">
                        </div>
                    </div>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white border border-default rounded-base shadow-sm p-4 md:p-6">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <form action="{{ route('admin.standar_mutu.hapus') }}" method="post">
                        @csrf
                        @method('delete')
                        <input type="text" name="standar_mutu_id" id="standar_mutu_id_hapus" hidden>
    ...
    <script>
        $(document).on('click', '.button-edit', function() {
            let standar_mutu_id = $(this).data('id');
            let nama_standar_mutu = $(this).data('nama');

            $('#standar_mutu_id').val(standar_mutu_id);
            $('#nama_standar_mutu').val(nama_standar_mutu);
        ...
        $(document).on('click', '.button-hapus', function() {
            let standar_mutu_id = $(this).data('id');
            $('#standar_mutu_id_hapus').val(standar_mutu_id);
